...
export function createServer() {
  ...
  server.registerTool(
    ...
    {
      description: "Get teachers that currently have meeting availability.",
      inputSchema: z.object({}),
    },
    async () => {
      const result = await getRicevimentoDocenti();
      return toolResult("Teachers with availability", { items: result, count: result.length });
    },
  );
  ...
  server.registerTool(
    ...
    {
      description: "Get currently available meeting slots for a teacher returned by get_ricevimento_docenti.",
      inputSchema: z.object({
        pkDocente: z.string().min(1, "pkDocente is required"),
      }),
    },
    async ({ pkDocente }) => {
      const result = await getDisponibilitaDocente(pkDocente);
      return toolResult("Teacher availability", { items: result, count: result.length });
    },
  );
